            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                <form action="{{ route('admin.locations.update', $location->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="name" value="{{ old('name', $location->name) }}" required class="appearance-none border border-emerald-200 rounded-xl w-full py-2.5 px-3 text-[#1a4d33] leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm">
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="description" rows="4" required class="appearance-none border border-emerald-200 rounded-xl w-full py-2.5 px-3 text-[#1a4d33] leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm">{{ old('description', $location->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-5 mb-5">
                        <div>
                            ...
                            <input type="text" name="latitude" value="{{ old('latitude', $location->latitude) }}" required class="appearance-none border border-emerald-200 rounded-xl w-full py-2.5 px-3 text-[#1a4d33] leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm">
                        </div>
                        <div>
                            <label class="block text-[#1a4d33] text-sm font-bold mb-2">Longitude</label>
                            <input type="text" name="longitude" value="{{ old('longitude', $location->longitude) }}" required class="appearance-none border border-emerald-200 rounded-xl w-full py-2.5 px-3 text-[#1a4d33] leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm">
                        </div>
                    </div>

                    <div class="mb-8">
                        <label class="block text-[#1a4d33] text-sm font-bold mb-3">Foto (Opsional)</label>
                        @if($location->image)
                            <div class="mb-4 flex items-center gap-4 p-4 border border-gray-100 bg-[#fbfaf5]/50 rounded-xl">
                                <img src="{{ asset('storage/' . $location->image) }}" alt="Foto saat ini" class="h-20 w-20 object-cover rounded-lg shadow-sm border border-gray-200">
                                <p class="text-xs text-[#246343] font-medium">Ini adalah foto saat ini.<br/>Biarkan input di bawah kosong jika tidak ingin mengganti foto.</p>
                            </div>
                        @endif
