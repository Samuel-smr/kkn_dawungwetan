...
<head>
    ...
    <title>Tambah Kategori - Admin Profil Desa</title>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                <form action="{{ route('admin.categories.store') }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="appearance-none border border-emerald-200 rounded-xl w-full py-2.5 px-3 text-[#1a4d33] leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-sm" placeholder="Contoh: Fasilitas Pendidikan">
                    </div>

                    <div class="mb-8">
                        <label class="block text-[#1a4d33] text-sm font-bold mb-2">Warna Kategori (Untuk Peta)</label>
                        <div class="flex items-center gap-4">
                            <input type="color" name="color" value="{{ old('color', '#246343') }}" required class="h-12 w-24 p-1 rounded-lg border border-emerald-200 cursor-pointer shadow-sm">
